---
layout: post
title: "Analysing the World Cup 2022 with Expected Threat (xT)"
subtitle: ""
...
All the probabilities and matrix can be calculated from this dataset. We now seem ready and go ahead to calculate xT, but we need to do one more processing before doing so. That is, we need to calculate the total play time by each player. This is due to the fact that play time largely differs between players, some plays over 400 mins, some does so just over 90 mins. xT is an accumulated value, hence the more play time could indicate more xT. It is necessary to normalise xT per 90 mins of play time.  
...
* <a href="https://statsbomb.com/wp-content/uploads/2023/03/Gregory-Everett-et-al-Contextual-xT-using-Spatial-Event-Data.pdf" style="color: #008AFF;">Gregory Everett</a> et al. : for his presentation and research paper on Expected Threat with contextual information